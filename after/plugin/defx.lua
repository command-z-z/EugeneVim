...
vim.keymap.set(
	"n",
	"sf",
	"<Cmd>Defx -listed -resume -split=vertical -winwidth=30 -direction=topleft -columns=indent:mark:icon:icons:filename:git:size -buffer-name=tab`tabpagenr()` `expand('%:p:h')` -search=`expand('%:p')`<CR>",
	{ noremap = true, silent = true, desc = "Open Defx file explorer" }
)
